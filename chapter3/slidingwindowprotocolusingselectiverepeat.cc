...
network layer in order. Associated with each outstanding frame is timer. When timer
expires, only that frame is retransmitted, not all outstanding frames, as in protocol 5.
*/
...
#include "protocol.h"
boolean no_nak = true; // no nak has been set yet
seq_nr oldest_frame = MAX_SEQ + 1; // initial value is only for this simulator
...
void protocol6(void) {
  ...
  ack_expected = 0; // next ack expected on inbound stream
  ...
  while (true) {
    ...
    switch(event) {
      case network_layer_ready: // accept, save, and transmit new frame
      ...
      case frame_arrival: // data or control frame has arrived
        ...
        if (r.kind == data) {
          ...
          if (between(frame_expected, r.seq, too_far) && (arrived[r.seq%NR_BUFS] == false)) {
            // Frames may be accepted in any odder
      ...
      case cksum_err:
        ...
        break;
      case timeout:
        send_frame(data, oldest_frame, frame_expected, out_buf); // timed out
        break;
      case ack_timeout:
